		...
		int fromlen)
{
	...
	socket_data.ABinding = (SocketHandle *)calloc(1,sizeof(SocketHandle));
    socket_data.ABinding->Port = htons(from->sin_port);
    char *pTmp = inet_ntoa(from->sin_addr);
	if(pTmp)
		strcpy(socket_data.ABinding->IP,pTmp);
...
static void udpServerKillTaskCondition(TUDPServer* This,
